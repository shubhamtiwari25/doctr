...
class DBNet(_DBNet, nn.Module):
    ...
    def compute_loss(
        self, out_map: torch.Tensor, thresh_map: torch.Tensor, target: List[np.ndarray]
    ) -> torch.Tensor:
        ...
        seg_target, seg_mask = torch.from_numpy(targets[0]), torch.from_numpy(
            targets[1]
        )
        seg_target, seg_mask = seg_target.to(out_map.device), seg_mask.to(
            out_map.device
        )
        thresh_target, thresh_mask = torch.from_numpy(targets[2]), torch.from_numpy(
            targets[3]
        )
        thresh_target, thresh_mask = thresh_target.to(out_map.device), thresh_mask.to(
            out_map.device
        )
...
def _dbnet(
    ...
    **kwargs: Any,
    ...
    pretrained_backbone = pretrained_backbone and not arch.split("_")[1].startswith(
        "resnet"
    )
